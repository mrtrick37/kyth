...
# Write plymouthd.conf and plymouthd.defaults unconditionally so that both the
# Fedora and CachyOS paths ship a non-empty config. On Fedora, bootc regenerates
# the initramfs on first deployment and reads these files from the container's /etc.
